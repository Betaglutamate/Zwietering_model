...
def calculate_regression_long(long_df):
    ...
    fit for growth rate
    """
    long_df = long_df.groupby('variable').apply(calculate_regression).reset_index(drop = True)   
    return long_df
...
def analyze_plate(file, **kwargs):
    ...
    aligned_df_long = merged.groupby('variable').apply(align_df).reset_index(drop=True) 
...
#     run_length, run_index = longest_run(df['smoothed_GR'].values[0:100])
