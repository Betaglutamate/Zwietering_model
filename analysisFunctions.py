...
class HelperFunctions:
    ...
    def calculate_regression_long(long_df):
        ...
        fit for growth rate
        """
        long_df = long_df.groupby('variable').apply(HelperFunctions.calculate_regression).reset_index(drop = True)   
        return long_df
    ...
    def analyze_plate(file, **kwargs):
        ...
        plate_normalized = HelperFunctions.normalize_plate(plate_path)
        ...
        aligned_df_long = merged.groupby('variable').apply(HelperFunctions.align_df).reset_index(drop=True) 
        ...
        if keywords.get('plot'):
            ...
            HelperFunctions.generate_plots(aligned_df_long, root)

        aligned_df_long = HelperFunctions.calculate_regression(aligned_df_long)
